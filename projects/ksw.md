...
- KSW-SOP-306：初期版はフローチャート画像が英語のみ、後の修正で**英語原図の後ろに日本語のみの別画像を追加**していた（`media_parts`/`inline_shapes` が原文比+1の未説明差分）。**2026-08-14解消**：原図1枚へのoverlay式バイリンガル化に統合・別画像削除で原文と構造完全一致（audit全PASS・Word COM保存確認）。**教訓：英日を別画像に分けない**（構造監査のパーツ数が原文と不一致になり、QAで自動検出される）。
...
**注意**：
- 参照プレセデントがフローチャート英語のみ（SOP-306等）でも、それを「英語維持」の根拠にしない。必ずバイリンガル化する。
